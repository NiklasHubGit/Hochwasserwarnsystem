...
#für das Board 4
grep 7066E1FFFE0004EE completed.txt > HW4-completed.txt
cat HW4-completed.txt |  awk -F "," '{print $9 $14 $7}' | \
	awk -F '"' '{print $5" "$8" "$11 }' | \
	awk -F ":" '{print $0}' | cut -c 2- > HW4-distanceTime.txt
cat HW4-distanceTime.txt | awk -F " " '{print $1}' > HW4-Dis.txt 
cat HW4-distanceTime.txt | awk -F " " '{print $2}' | awk -F "." '{print $1}' | awk -F "T" '{print $1"_"$2}' \
	> HW4-Time.txt 
cat HW4-distanceTime.txt | awk -F " " '{print $3}' | cut -c 2- > HW4-Stamp.txt
